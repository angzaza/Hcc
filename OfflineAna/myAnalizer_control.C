...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  TH1F* PUrewHisto;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //cout<<"size puppi jets: "<<AK4PuppiJets_pt->size()<<endl;
      //cout<<"size PNet score: "<<jet_pfParticleNetAK4JetTags_probc->size()<<endl;
      HLT_passed=false;
      ...
      if(HLT_Control==true && AK4PuppiJets_pt->size()>=4){
         ...
         float pt_offlineThr[4]={105.,88.,70.,35.};
      ...
      //if((HLT_Signal == true || HLT_Control==true) && AK4PuppiJets_pt->size()>=4){
        for(unsigned int i=0; i<4; i++){
          ...
          //cout<<"pt: "<<AK4PuppiJets_pt->at(i)<<"    eta: "<<AK4PuppiJets_eta->at(i)<<"      PNet: "<<jet_pfParticleNetAK4JetTags_probc->at(i)/(jet_pfParticleNetAK4JetTags_probc->at(i)+ jet_pfParticleNetAK4JetTags_probb->at(i) + jet_pfParticleNetAK4JetTags_probuds->at(i) + jet_pfParticleNetAK4JetTags_probg->at(i))<<endl;
          ...
          if(i!=3){
            for(unsigned int j=0; j<HLTAK4PFJetLoose_pt->size(); j++){
              //if(i==0) cout<<"hlt jet "<<j<<"     pt: "<<HLTAK4PFJetLoose_pt->at(j)<<"      eta: "<<HLTAK4PFJetLoose_eta->at(j)<<endl;
        ...
        if(AK4puppi_PNet.size()>=4){
          ...
          if(AK4puppi_cHiggs.size()==2 && AK4puppi_VBF.size()==2 && (AK4puppi_VBF[0].jet_p4+AK4puppi_VBF[1].jet_p4).M()>=500. && fabs(AK4puppi_VBF[0].jet_p4.Eta() -AK4puppi_VBF[1].jet_p4.Eta())>=3.8){
            histoPNet_control->Fill(AK4puppi_cHiggs.at(0).CvsAll,pu_weight);
            //cout<<"histogram filled with CvsAll: "<<AK4puppi_cHiggs.at(0).CvsAll<<endl;
            if(HLT_Signal==true){
              //cout<<"fired hlt signal"<<endl;
              histoPNet_signal->Fill(AK4puppi_cHiggs.at(0).CvsAll,pu_weight);
            }
          }
        }
      ...
      //if(HLT_passed==true && jet_pfParticleNetAK4JetTags_probc->size()>=3){ //TO BE CHANGED WITH UPPER LINE
        ...
        if(tag_passed==true){
          ...
          //cout<<"pt jet2: "<< AK4puppi_jet2.Pt()<<endl;   
          //cout<<"pt jet3: "<< AK4puppi_jet3.Pt()<<endl;   
          histoPtEta_jet1_tag->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta())); //denominator
          histoPtEta_jet2_tag->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta())); //denominator
          if(HLT_PFJet60_passed==true){
            histoPtEta_jet3_tag->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta())); //denominator
          }
          //cout<<"size hlt jets: "<<HLTAK4PFJetLoose_pt->size()<<endl;
          if(HLT_PFJet60_passed==true){
          ...
          if(probe3_passed==true) histoPtEta_jet3_probe->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta()));
          if(probe2_passed==true) histoPtEta_jet2_probe->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta()));
          if(probe1_passed==true) histoPtEta_jet1_probe->Fill(AK4puppi_jet2.Pt(), fabs(AK4puppi_jet2.Eta()));
