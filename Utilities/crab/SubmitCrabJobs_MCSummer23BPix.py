...
def parseOptions():
    ...
    parser.add_option('-d', '--datasets', dest='DATASETS', type='string', default='dataset_MC_Summer23BPix.txt', help='txt file with datasets to run over')
    parser.add_option('-c', '--cfg', dest='CONFIGFILE', type='string', default='/afs/cern.ch/work/a/azaza/HccAna/CMSSW_13_0_13/src/Hcc/HccAna/python/crab/crabConfig_TEMPLATE_MCSummer23BPix.py', help='configuration template')
...
def submitAnalyzer():
    ...
    for dataset in datasets:
        ...
        if (len(filename)>99):
          ...
          newfilename = filename.split('-130X')[0]
        ...
        cmd = 'cp '+cfgtemplate+' '+outDir+'/cfg/'+crabcfgfile
        output = processCmd(cmd)
        ...
        #cmd = "sed -i 's~CFGFILE~"+cfgtemplate
        output = processCmd(cmd)
